...
const char gszDiabloShrinesText[] = {
	"$Abandoned Shrine|"
	"Text:  The hands of men may be guided by fate|"
	"Effect: +2 Dexterity|"
	"|"
	"$Blood Fountain|"
	"Text:  None|"
	"Effect: Each click heals 1 HP|"
	"|"
	"$Cauldron|"
	"Text:  Random|"
	"Effect: Random|"
	"|"
	"$Creepy Shrine|"
	"Text:  Strength is bolstered by heavenly faith|"
	"Effect: +2 Strength|"
	"|"
	"$Cryptic Shrine|"
	"Text:  Arcane power brings destruction|"
	"Effect: Restores Mana and casts Nova|"
	"|"
	"$Divine Shrine|"
	"Text:  Drink and be Refreshed|"
	"Effect: Restores HP and Mana, drops 2 potions|"
	"|"
	"$Eerie Shrine|"
	"Text:  Knowledge and wisdom at the cost of self|"
	"Effect: +2 Magic|"
	"|"
	"$Eldritch Shrine|"
	"Text:  Crimson and azure become as the sun|"
	"Effect: Changes all potions into rejuvenation potions|"
	"|"
	"$Enchanted Shrine|"
	"Text:  Magic is not always what it seems to be|"
	"Effect: -2 to a random spell, +1 to all other spells if possible|"
	"|"
	"$Fascinating Shrine|"
	"Text:  Intensity comes at the cost of wisdom|"
	"Effect: -10% of Max Mana, +2 lvl to Firebolt spell|"
	"|"
	"$Fountain of Tears|"
	"Text:  None|"
	"Effect: -1 to a random attribute, +1 to an other|"
	"|"
	"$Glimmering Shrine|"
	"Text:  Mysteries are revealed in the light of reason|"
	"Effect: Identifies all items in inventory|"
	"|"
	"$Goat Shrine|"
	"Text:  Random|"
	"Effect: Random|"
	"|"
	"$Gloomy Shrine|"
	"Text:  Those who defend seldom attack|"
	"Effect: +2 AC to all armors, helms and shields in inventory|"
	"       -1 to max damage to all weapons in inventory|"
	"Note:  Single-player only|"
	"|"
	"$Hidden Shrine|"
	"Text:  New strength is forged through destruction|"
	"Effect: -10 max durability to a random equipped item|"
	"       +10 max durability to all others|"
	"|"
	"$Holy Shrine|"
	"Text:  Wherever you go, there you are|"
	"Effect: Casts Phasing|"
	"|"
	"$Magical Shrine|"
	"Text:  While the spirit is vigilant the body thrives|"
	"Effect: Casts Mana Shield|"
	"|"
	"$Murky Pool|"
	"Text:  None|"
	"Effect: Casts Infravision|"
	"|"
	"$Mysterious Shrine|"
	"Text:  Some are weakened as one grows strong|"
	"Effect: +5 to a random attribute. -1 to all other|"
	"|"
	"$Ornate Shrine|"
	"Text:  Salvation comes at the cost of Wisdom|"
	"Effect: -10% of Max Mana, +2 lvl to Holy Bolt spell|"
	"|"
	"$Purifying Spring|"
	"Text:  None|"
	"Effect: Each click restores 1 Mana point|"
	"|"
	"$Quiet Shrine|"
	"Text:  The essence of life flows from within|"
	"Effect: +2 Vitality|"
	"|"
	"$Religious Shrine|"
	"Text:  Time cannot diminish the power of steel|"
	"Effect: Repairs all items|"
	"|"
	"$Sacred Shrine|"
	"Text:  Energy comes at the cost of wisdom|"
	"Effect: -10% of Max Mana, +2 lvl to Charged Bolt spell|"
	"|"
	"$Secluded Shrine|"
	"Text:  The way is made clear when viewed from above|"
	"Effect: Shows complete map of the current level|"
	"|"
	"$Spiritual Shrine|"
	"Text:  Riches abound when least expected|"
	"Effect: Fill empty cases in inventory with some gold|"
	"|"
	"$Spooky Shrine|"
	"Text:  Where avarice fails, patience brings reward (user)|"
	"       Blessed by a benevolent companion (others)|"
	"Effect: (user) No effect|"
	"       (others) Health and Mana are restored to full|"
	"|"
	"$Stone Shrine|"
	"Text:  The powers of mana refocused renews|"
	"Effect: Recharges all staves|"
	"|"
	"$Tainted Shrine|"
	"Text:  Those who are last may yet be first (user)|"
	"       Generosity brings its own reward (others)|"
	"Effect: (user) No effect|"
	"       (others) +1 to a random attribute, -1 to all other|"
	"|"
	"$Thaumaturgic Shrine|"
	"Text:  What was once opened now is closed|"
	"Effect: Closes and refills all opened chests on current level|"
	"Note:  Single-player only|"
	"|"
	"$Weird Shrine|"
	"Text:  The sword of justice is swift and sharp|"
	"Effect: +1 to max damage of all weapons in inventory|"
	"&"
};

const char gszHellfireShrinesText[] = {
	"$Abandoned Shrine|"
	"Text:  The hands of men may be guided by fate|"
	"Effect: +2 Dexterity|"
	"|"
	...
	"Effect: Converts half of your gold to experience|"
	"|"
	"$Murky Pool|"
	"Text:  None|"
	"Effect: Casts Infravision|"
	"|"
	"$Murphy's Shrine|"
	"Text:  That which can break, will|"
	"Effect: 1 to 3 chance to lose 50% durability,|"
	"       of a random equipped item,|"
	"       or takes away 1/3 of the gold in inventory|"
	"|"
	"$Mysterious Shrine|"
	"Text:  Some are weakened as one grows strong|"
	"Effect: +5 to a random attribute. -1 to all other|"
	...
	"|"
	"$Oily Shrine|"
	"Text:  That which does not kill you...|"
	"Effect: +2 Strength (Warrior) / +2 Dexterity (Rogue)|"
	"       +2 Magic (Sorcerer) / +1 Strength and +1 Dexterity (Monk)|"
	"       +1 Dexterity and +1 Magic (Bard) / +2 Vitality (Barbarian)|"
	"|"
	"$Purifying Spring|"
	"Text:  None|"
	"Effect: Each click restores 1 Mana point|"
	...
	"|"
	"$Shimmering Shrine|"
	"Text:  Spiritual energy is restored|"
	"Effect: Restores Mana to full|"
	"|"
	"$Solar Shrine|"
	"12pm - 6pm|"
	...
	"Effect: +2 Vitality|"
	"4am - 12pm|"
	"Text:  You feel more agile|"
	"Effect: +2 Dexterity|"
	"|"
	"$Sparkling Shrine|"
	"Text:  Some experience is gained by touch|"
	"Effect: Gives 1000x(dungeon level) experience points, casts a Flash|"
	"|"
	"$Spiritual Shrine|"
	"Text:  Riches abound when least expected|"
	"Effect: Fill empty cases in inventory with some gold|"
