...
StartPlayerKill(int pnum, int earflag)
{
	...
	bool diablolevel = *sgOptions.Gameplay.noDeathDrop || (gbIsMultiplayer && player.plrlevel == 16);
...
void CheckPlrSpell(bool isShiftHeld, spell_id spellID, spell_type spellType, bool quick)
{
	...
	if (!sgbControllerActive) {
		...
		if (
		    ((chrflag || stashflag || QuestLogIsOpen) && GetLeftPanel().Contains(MousePosition)) // inside left panel
